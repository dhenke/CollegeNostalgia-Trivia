...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	self.questionViewController = [[QuestionViewController alloc] initWithNibName:@"QuestionView" bundle:nil category:rowValue];
...
-(void) sendRequest{	
	...
	NSString *postBody = [NSString stringWithFormat:@"userID=%@", [array objectAtIndex:0]];
...
- (void)viewDidLoad {
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(removeCategoryView) name:@"RemoveCategoryView" object:nil];
	[self sendRequest];
	[listData retain];
    [super viewDidLoad];
}
